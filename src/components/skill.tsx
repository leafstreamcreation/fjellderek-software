  ...
  return (
    <div>
      <Progress 
        ...
        size="lg" 
      />
      <p className="mx-2 text-wrap text-left text-default-300">{description}</p>
    </div>
  );
};
...
export function SubSkills({ skill }: { skill: Skill }) {
  ...
  function SubSkillDescription({ subSkill }: { subSkill: Skill }) {
    ...
    return description ? <p className="text-wrap text-left mr-4 text-default-300">{description}</p> : null;
  };
